...
let pivot_optimal t =
  ...
  (* contruction d'un tableau donnant une valeur pour comparer le nombre de valeur supérieurs et inférieurs pour chaque indices.*)
  for i = 0 to lon do
    for j = 0 to lon do
      cpt := !cpt +1;
      if t.t.(i) < t.t.(j) then
  ...
  (* Choix de l'indice qui a le nombre de valeurs supérieurs et inférieurs le plus proche *)
  let ind = ref 0 in
  for k = 1 to lon do
      cpt := !cpt +1;
