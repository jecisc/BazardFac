...
public class XMLTOOLS1 {
	...
	public static void ecrireXML(Document document, String nameFile,
			String DTDFile) {
		try {
	...
	public static void xslTranform(String xslNameFile, String xmlNameFile,
			String outNameFile) {
		try {
			TransformerFactory factory = TransformerFactory.newInstance();
			Transformer transfXSLT = factory.newTransformer(new StreamSource(
					xslNameFile));
			...
			transXML.transform(new DOMSource(out.getNode()), new StreamResult(
					outNameFile));
		} catch (TransformerException e) {
